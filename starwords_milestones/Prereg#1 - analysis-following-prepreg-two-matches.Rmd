...
kids <- read.csv2("kids_to_share.csv")
str(kids)
...
```{r supplementary tables, include = T, echo = F}
...
1. RQ1: Will bilinguals reach particular motor and language milestones at a similar time as their Polish monolingual peers, **at least in one of their languages**?  
...
words_to_share <- read.csv2("words_to_share.csv")

words_rq1 <- words_to_share %>%
...
```{r Multiwords RQ1}
multiwords_to_share <- read.csv2("multiwords_to_share.csv")

# long to wide
multiwords_wider <- multiwords_to_share %>% ungroup() %>%
...
milestones_to_share <- read.csv2("milestones_to_share.csv")
milestones_to_share <- milestones_to_share %>% select(-X)

rq1 <- merge(rq1, milestones_to_share[,c("child_anonim", "crawling_age", "walking_age", "babbling_age")], by = "child_anonim", all.x = T)
...
These analyses include the whole bilingual group (n = 302), irregardless of their language input patterns (for a subset of children, see Appendix B).


*Note 1.* For milestones regarding words and utterances milestones: we use the number of words reported by the parent in the app.  
*Note 2.* If a word/utterance was marked as belonging to two languages (e.g. Polish and English), we count the word twice, for the Polish milestones and the English milestones. 
...
multiwords_longer <- multiwords_to_share %>% 
  arrange(child_anonim) %>%
  select(-c(m_cat)) %>%
  pivot_longer(cols = c("l1", "l2", "l3"),
               names_to = "language_nr",
...
# clean - step 1 - remove children with no data 
lineplot_rq2 <- lineplot_rq2 %>%
  filter(!is.na(language_new))
# (down from 302 to 142 children!)
length(unique(lineplot_rq2$child_anonim))
...
rq2_w1 <- rq2_w1 %>% filter(child_anonim != 4730)
rq2_w1 %>% ggplot(aes(x = language_new, y = age_at_element)) + geom_boxplot() + geom_jitter()
...
# Figure 2 - data from all bilinguals (irregardless of input properties)
...
# words for Appendix 1 (A1)

words_A1 <- words_to_share %>%
  filter(word_number == 1 | word_number == 10 | word_number == 50) %>%
  select(c(child_anonim, group, match, word_number, age_at_element))

# zamieniamy na wide, tak żeby pasował do rq1 df
words_A1_wider <- words_A1 %>%
...
#  multiwords for Appendix 1 (A1)
multiwords_A1 <- multiwords_to_share %>%
  filter(!is.na(m_cat)) %>%
...
```{r}
# add milestones
milestones_A1 <- milestones_to_share

# merge words, mulytiwords and milestones into one data frame
all_A1 <- merge(milestones_A1, words_A1_wider, by = "child_anonim", all = T)
all_A1 <- merge(all_A1, multiwords_A1[,c("child_anonim", "m1", "m3", "m4")], by = "child_anonim", all = T)
...
# wide to long
lineplot_A_longer <- all_A1 %>%
  pivot_longer(cols = 4:12, names_to = "milestone", values_to = "age_at_element")

lineplot_A_longer <- lineplot_A_longer %>% unique()

lineplot_A_longer$milestone <- factor(lineplot_A_longer$milestone, ordered = T, levels = c("crawling_age", "babbling_age", "walking_age","w1", "w10", "w50", "m1", "m3", "m4"))
...
```{r App.A outliers}
# view the data for potential outliers
lineplot_A_longer %>% ggplot(aes(x = milestone, y = age_at_element, fill = group)) + geom_boxplot() + geom_jitter() +
  theme(legend.position = "bottom")

# identify outliers (rstatix::identify...)
A_outliers <- lineplot_A_longer %>%
  dplyr::group_by(group, milestone) %>% 
  identify_outliers("age_at_element")

# mark outliers in the dataset and filter out
lineplot_A_longer <- merge(lineplot_A_longer, A_outliers[, c("child_anonim", "milestone", "is.outlier")], all.x = T)

lineplot_A_longer <- lineplot_A_longer %>% filter(is.na(is.outlier))
...
lineplot_A_sum <- summarySE(lineplot_A_longer, measurevar = "age_at_element", groupvars = c("milestone", "group"), na.rm = T)
lineplot_A_sum$milestone <- as.factor(lineplot_A_sum$milestone)

lineplot_A_sum <- lineplot_A_sum %>%
  mutate(source = dplyr::case_when(
...
plot_grid(figAa, figAb, title, align = "h",  rel_widths = c(1,2), rel_heights = c(1, .1))
ggsave("fig-A1.jpeg", dpi = 250, width = 2100, height = 1000, units = "px")
...
#### Appendix A: one-way ANOVAs
Significant only for 1st 4-word utterance, but small sample sizes.
```{r Supp ANOVA, include = T, echo=FALSE, warning=FALSE}

aov_partial <- lineplot_A_longer %>%
...
```
#### Appendix A: Bayes Factor

```{r, include = T, echo= T, warning=FALSE}
lineplot_A <- lineplot_A_longer %>%
...
# crawling
prepbf <-  lm(crawling_age~group-1, data=lineplot_A) #Without the -1 regular dummy coding would have been applied
coef(prepbf)
set.seed(100)
...
# babbling
prepbf <-  lm(babbling_age~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# walking
prepbf <-  lm(walking_age~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# word1
prepbf <-  lm(w1~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# word10
prepbf <-  lm(w10~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# word50
prepbf <-  lm(w50~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# multiword1
prepbf <-  lm(m1~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
# multiword3
prepbf <-  lm(m3~group-1, data=lineplot_A)
coef(prepbf)
set.seed(100)
resbf<-bain(prepbf,"groupBilinguals = groupMonolinguals1 = groupMonolinguals2")
print(resbf)
...
```{r, include = T, echo=FALSE, warning=FALSE}
input <- read.csv2("input_kto.csv")

input <- input %>%
  dplyr::select("child_anonim", "group", "group_raw", "home_input")

input$home_input <- factor(input$home_input, levels = c("noInfo", "mixed", "polish"))
input$group_raw <- factor(input$group_raw, levels = c("BI-UK", "BI-NO", "BI-X"))

input_sum <- input %>%
  group_by(group_raw, home_input) %>%
  dplyr::summarise(n = length(child_anonim))

input_sum <- input_sum %>% dplyr::group_by(group_raw) %>% mutate(percentage=100*n/sum(n))
...
input_sum %>%
  ggplot(aes(y = percentage, x = group_raw, fill = home_input)) +
  geom_bar(stat="identity") + 
  geom_text(aes(x = group_raw, y = percentage, label = paste0(percentage,"%")), size=4,
...
ggsave("figB1.jpeg", height = 1400, units = "px", dpi = 300)

```

```{r children from pre-registered rq2}

B_all <- merge(rq2, input[, c("child_anonim", "group_raw", "home_input")], by = "child_anonim", all.x = T)
...
B_w1 <- B_all %>%
  dplyr::select("child_anonim", "language_new", "w1", "group_raw", "home_input") %>%
  pivot_wider(names_from = language_new, values_from = w1) %>%
  filter(complete.cases(HL, ML)) %>%
  pivot_longer(cols = c(HL, ML), names_to = "language_new", values_to = "age_at_element") %>%
  mutate(milestone = "w1") %>% dplyr::relocate(milestone, .before = child_anonim)
...
```{r, include = T, echo=FALSE, warning=FALSE}
# we take only the vars we want for RQ2
lineplot_B <- B_all %>% filter(group_raw == "BI-UK" | group_raw == "BI-NO")
...
B_w1_se <- B_w1 %>%
  summarySE(measurevar = "age_at_element", groupvars = c("language_new", "group_raw"), na.rm = T) %>%
  mutate(milestone = "w1") %>% dplyr::relocate(milestone, .before = language_new)

#w10
B_w10 <- lineplot_B %>%
  dplyr::select("child_anonim", "language_new", "w10", "group_raw", "home_input") %>%
  pivot_wider(names_from = language_new, values_from = w10) %>%
  filter(complete.cases(HL, ML)) %>%
  pivot_longer(cols = c(HL, ML), names_to = "language_new", values_to = "age_at_element") %>%
  mutate(milestone = "w10") %>% dplyr::relocate(milestone, .before = child_anonim)
...
#none
B_w50 <- lineplot_B %>%
  dplyr::select("child_anonim", "language_new", "w50", "group_raw", "home_input") %>%
  pivot_wider(names_from = language_new, values_from = w50) %>%
  filter(complete.cases(HL, ML)) %>%
  pivot_longer(cols = c(HL, ML), names_to = "language_new", values_to = "age_at_element") %>%
  mutate(milestone = "w50") %>% dplyr::relocate(milestone, .before = child_anonim)
...
#m1
B_m1 <- lineplot_B %>%
  dplyr::select("child_anonim", "language_new", "m1", "group_raw", "home_input") %>%
  pivot_wider(names_from = language_new, values_from = m1) %>%
  filter(complete.cases(HL, ML)) %>%
  pivot_longer(cols = c(HL, ML), names_to = "language_new", values_to = "age_at_element") %>%
  mutate(milestone = "m1") %>% dplyr::relocate(milestone, .before = child_anonim)
...
#m3
B_m3 <- lineplot_B %>%
  dplyr::select("child_anonim", "language_new", "m3", "group_raw", "home_input") %>%
  pivot_wider(names_from = language_new, values_from = m3) %>%
  filter(complete.cases(HL, ML)) %>%
  pivot_longer(cols = c(HL, ML), names_to = "language_new", values_to = "age_at_element") %>%
  mutate(milestone = "m3") %>% dplyr::relocate(milestone, .before = child_anonim)
...
# summary
lineplot_B_sum <- rbind(B_w1_se, B_w10_se, B_m1_se)
