...
lineplot_rq1_sum <- lineplot_rq1_sum %>%
  mutate(source = dplyr::case_when(
    milestone == "babbling" ~ "vocabulary milestone",
...
plot_grid(fig1a_optB, fig1b_optB, align = "h",  rel_widths = c(1,1), rel_heights = c(1, .1))
ggsave("fig1.jpeg", dpi = 250, width = 1900, height = 1000, units = "px")
```



##### Figure 1 option B for BMS:
[x] panel with with vocabulary milestones and utterance milestones

```{r Figure 1 option B for BMS}
#summary
lineplot_rq1_sum <- summarySE(lineplot_rq1_longer, measurevar = "age_at_element", groupvars = c("milestone", "group"), na.rm = T)
...
fig1a_optB <- lineplot_rq1_sum %>%
  filter(source == "vocabulary milestone") %>%
  ggplot(aes(x=milestone, y=age_at_element, group=group, color=group)) + 
  geom_line(linetype = 2, color = "black", alpha = .3, size = 0.8) +
  geom_point(position=position_dodge(0.03), size = 2) +
  geom_errorbar(aes(ymin=age_at_element-ci, ymax=age_at_element+ci),
                width=0.5, 
                position=position_dodge(0.03), size = 0.8) +
  scale_y_continuous(limits = c(4,28), breaks = c(4, 8, 12, 16, 20, 24, 28)) + 
  scale_x_discrete(labels = c("babbling" = "Babbling \n B n = 58 \n M n = 54",
                              "w1" ="1st word \n B n = 140 \n M n = 137",
                              "w10" = "10th word  \n B n = 50 \n M n = 47",
...
plot_grid(fig1a_optB, fig1b_optB, align = "h",  rel_widths = c(1,1), rel_heights = c(1, .1))
ggsave("fig1_for_BMS.jpeg", dpi = 600, width = 190, height = 100, units = "mm")
```
...
[x] remove linetype by group and make both lines grey/dotted (and add transparency)
[x] larger breaks on the y axis? maybe by 6 months? --> MK: did 4 months
[x] seperate plots for vocabulary and utterance milestones
...
#divide into vocabulary and utterance milestones
lineplot_sum_rq2 <- lineplot_sum_rq2 %>%
  mutate(source = dplyr::case_when(
    milestone == "w1" ~ "vocabulary milestone",
    milestone == "w10" ~ "vocabulary milestone",
    milestone == "w50" ~ "vocabulary milestone",
...
#vocabulary milestones
fig2a_alt <- lineplot_sum_rq2 %>%
  filter(source == "vocabulary milestone") %>%
  ggplot(aes(x=milestone, y=age_at_element, group=language_new, color=language_new)) + 
  geom_line(linetype = 2, color = "black", alpha = .3) +
  geom_point(position=position_dodge(0.03)) +
  geom_errorbar(aes(ymin=age_at_element-ci, ymax=age_at_element+ci),
                width=0.5, 
...
legend_right <- get_legend(
  # create some space to the left of the legend
  fig2b_alt + theme(legend.box.margin = margin(0, 0, 0, 50)))

plot_grid(fig2a_alt, fig2b_alt, align = "h",  rel_widths = c(1,1), rel_heights = c(1, .1))
ggsave("fig2.jpeg", dpi = 250, width = 1900, height = 1000, units = "px")
...
```{r}
# Figure 2 for BMS - data from all bilinguals (with various majority languages) 
...
plot_grid(fig2a_alt, fig2b_alt, align = "h",  rel_widths = c(1,1), rel_heights = c(1, .1))
ggsave("fig2_for_BMS.jpeg", dpi = 400, width = 190, height = 100, units = "mm")
...
[x] remove crawling and walking but keep babbling in
[x] remove the two facets (panels) and merge into a single plot (and remove caption/title on top)
[x] maybe remove linetype by group and make both lines grey/dotted (and add transparency)
[x] larger breaks on the y axis? maybe by 6 months? --> MK: did 4 months
[x] seperate plots for vocabulary and utterance milestones

```{r Figure B1 option B}

#divide into vocabulary and utterance milestones
lineplot_B_sum_filtered <- lineplot_B_sum_filtered %>%
  mutate(source = dplyr::case_when(
    milestone == "babbling_age" ~ "vocabulary milestone",
    milestone == "crawling" ~ "parental recall",
    milestone == "walking" ~ "parental recall",
    milestone == "w1" ~ "vocabulary milestone",
    milestone == "w10" ~ "vocabulary milestone",
...
#vocabulary milestones
figB1a_optB <- lineplot_B_sum_filtered %>%
  filter(source == "vocabulary milestone") %>%
  ggplot(aes(x=milestone, y=age_at_element, group=group, color=group)) + 
  geom_line(linetype = 2, color = "black", alpha = .3) +
  geom_point(position=position_dodge(0.03)) +
...
#utterance milestones
figB1b_optB <- lineplot_B_sum_filtered %>%
  filter(source == "utterance milestone") %>%
  ggplot(aes(x=milestone, y=age_at_element, group=group, color=group)) + 
  geom_line(linetype = 2, color = "black", alpha = .3) +
  geom_point(position=position_dodge(0.03)) +
...
legend_right <- get_legend(
  # create some space to the left of the legend
  figB1b_optB + theme(legend.box.margin = margin(0, 0, 0, 50)))

plot_grid(figB1a_optB, figB1b_optB, align = "h",  rel_widths = c(1,1), rel_heights = c(1, .1))
ggsave("figB1.jpeg", dpi = 250, width = 1900, height = 1000, units = "px")
```
